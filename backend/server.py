...
# Health check endpoint for Kubernetes probes (at root level, not under /api)
@app.get("/health")
async def health_check():
    """Health check endpoint for Kubernetes liveness/readiness probes"""
    try:
        # Quick MongoDB ping to verify database connectivity
        await db.command("ping")
        return {"status": "healthy", "database": "connected"}
    except Exception as e:
        return {"status": "unhealthy", "database": "disconnected", "error": str(e)}

# Also add under /api for ingress routing
...
class AdPosition(str, Enum):
    HOME_BANNER = "home_banner"          # Large banner on homepage
    HOME_SIDEBAR = "home_sidebar"        # Sidebar ad on homepage
    PRODUCT_INLINE = "product_inline"    # Between product listings
    PRODUCT_SIDEBAR = "product_sidebar"  # Sidebar on product pages
    FOOTER = "footer"                    # Footer banner
    POPUP = "popup"                      # Popup ad

class AdCreate(BaseModel):
    name: str
    position: str  # AdPosition value
    image_url: str
    target_url: str
