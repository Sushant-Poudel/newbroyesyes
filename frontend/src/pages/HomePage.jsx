    ...
    <div className="min-h-screen bg-black">
      ...
      <section className={`${hasNotification ? 'pt-20 lg:pt-24' : 'pt-20 lg:pt-24'}`} data-testid="reviews-section">
        ...
        <div className="py-6 lg:py-8 overflow-hidden">
          ...
          {isLoading ? (
            <div className="flex gap-4 lg:gap-6 px-4">{[1, 2, 3, 4].map((i) => <div key={i} className="h-32 lg:h-40 w-72 lg:w-80 glass-card rounded-2xl flex-shrink-0 animate-pulse"></div>)}</div>
          ) : reviews.length > 0 ? (
        ...
        <section className="py-8 lg:py-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/" className="text-amber-500 text-sm hover:underline flex items-center gap-1">View All <ChevronRight className="h-4 w-4" /></Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4">{hotDeals.map((product) => <ProductCard key={product.id} product={product} />)}</div>
          </div>
        </section>
      )}
      ...
      {newArrivals.length > 0 && (
        <section className="py-8 lg:py-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4">{newArrivals.map((product) => <ProductCard key={product.id} product={product} />)}</div>
          </div>
        </section>
      )}
      ...
      <section ref={productsSectionRef} className="py-8 lg:py-12" data-testid="products-section">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-6 lg:mb-8">
            <h2 className="font-heading text-3xl sm:text-4xl md:text-5xl font-bold text-white uppercase tracking-tight mb-2 lg:mb-3 animate-fade-in">All <span className="text-amber-500">Products</span></h2>
          ...
          {searchQuery && (
            <div className="flex items-center justify-center gap-2 mb-4 lg:mb-6 animate-scale-in">
              <div className="flex items-center gap-2 glass rounded-full px-4 py-2">
                ...
                <span className="text-white text-sm">Results for "<span className="text-amber-500">{searchQuery}</span>"</span>
          ...
          {isLoading ? (
            ...
            <div className="flex gap-6">
              {/* Products Grid */}
              <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-5">
